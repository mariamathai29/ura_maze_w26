k# URA Maze Project, Winter 2026 Updates
...
We also attempted to control maze conditions such as path length via reinforcement learning. We used two methods; firstly, a Gaussian distribution for the reward depending on how close the path length was to the desired L (gaussian_length.ipynb); secondly, a tiered binary reward system, giving some reward for solvability, and some for matching path length.
